...
import torch
import numpy as np
from ...subNets_diffusion.transformers_encoder.transformer import TransformerEncoder
...
def loss_fn(model, x, marginal_prob_std, condition=None, eps=1e-5):
  ...
  loss = torch.mean(torch.sum((score * std[:, None, None] + z)**2, dim=(1,2)))#得分网络的损失，公式18
  return loss
    ...
    with torch.no_grad():
        for time_step in time_steps:
            ...
            if condition is not None:
                ...
                mean_x = x + (g ** 2)[:, None, None] * score_model(x, batch_time_step, perturbed_condition) * step_size   #根据时间步生成去噪的数据，公式16
            else:
                mean_x = x + (g ** 2)[:, None, None] * score_model(x, batch_time_step) * step_size
            x = mean_x + torch.sqrt(step_size) * g[:, None, None] * torch.randn_like(x)#公式16
